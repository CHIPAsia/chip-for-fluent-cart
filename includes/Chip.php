...
class Chip extends AbstractPaymentGateway {
					...
					if ( $paymentData && 'paid' === $paymentData['status'] ) {
						...
						$orderTransaction->fill(
							array(
								...
								'payment_method_type' => $this->mapPaymentMethodType( $paymentData['payment_method_type'], $paymentData['transaction_data'] ?? array() ),
							)
						);
	...
	protected function processPayment( $paymentData ) {
		try {
			...
			$products   = array();
			$orderItems = $paymentData['order_items'] ?? array();
			...
			$initUrl         = $this->getInitUrl( (object) array( 'uuid' => $transactionUuid ) );
	...
	protected function getPublicKey( $brandId ) {
		...
		$storedData = get_option( self::PUBLIC_KEY_OPTION, array() );
	...
	protected function processWebhook( $webhookData ) {
		...
		$transactionData   = $webhookData['transaction_data'] ?? array();
	...
	protected function checkPaymentStatus( $purchaseId ) {
		...
		try {
			...
			if ( $payment && isset( $payment['status'] ) ) {
				$transactionData = $payment['transaction_data'] ?? array();
				return array(
	 ...
	 */
	protected function mapPaymentMethodType( $paymentMethod, $transactionData = array() ) {
